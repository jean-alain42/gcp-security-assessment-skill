...
async function main() {
    ...
    failedRows.forEach(row => {
        // Resolve resource ID and fallback
    ...
    // Read the detailed Report.md as description or generate one if missing
    ...
    // 2. Post Resource-Specific Results
    const resourceResults = Object.values(resourceMap).map(res => {
        // Compile a clean combined recommendation string
        const recommendationStr = Array.from(res.recommendations)
            .slice(0, 3) // limit to top 3 recommendations to keep it readable
            .join("; ");
