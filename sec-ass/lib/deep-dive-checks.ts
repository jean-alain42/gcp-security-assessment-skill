  ...
  try {
    ...
    for (const line of clusters.split('\n')) {
      ...
      try {
        ...
        // GKE-Specific Toxic Combination Check
        if ((privilegedPods.length > 0 || rootPods.length > 0) && netpolCount === 0 && adminBindings.length > 0) {
          results += `\n*   ⚠️ **GKE TOXIC COMBINATION DETECTED (Critical Risk):**\n`;
          results += `    *   This GKE cluster exhibits a **Highly Critical Toxic Combination**: containers are running with privileged/root permissions, combined with **Zero Network Policies** (no lateral isolation) and **unrestricted cluster-admin bindings**.\n`;
          results += `    *   **Threat Vector (Kill Chain):** An attacker compromising a single pod can break out of container boundaries to node root access, move laterally across the internal network to other namespaces, capture service account tokens, and utilize the wide-open \`cluster-admin\` RBAC bindings to completely take over the control plane of the entire Kubernetes cluster.\n`;
          results += `    *   **Remediation:** Enforce network policies immediately to isolate namespaces, apply Pod Security Admissions to block privileged containers (\`runAsNonRoot: true\`), and down-scope non-system users/roles from the \`cluster-admin\` role.\n`;
        }
