...
function analyzeToxicCombinations(aggregatedData: Record<string, AggregatedFinding>): string {
  let sections = "";
  
  // 1. Compute Permissive Ingress + Broad Service Account
  const hasPublicSSH = Object.values(aggregatedData).some(f => f.CHECK_ID.includes('firewall_ssh_access_from_the_internet_allowed'));
  const hasPublicRDP = Object.values(aggregatedData).some(f => f.CHECK_ID.includes('firewall_rdp_access_from_the_internet_allowed'));
  const hasPermissiveIngress = hasPublicSSH || hasPublicRDP;
  
  const hasBroadSA = Object.values(aggregatedData).some(f => 
    f.CHECK_ID.includes('no_administrative_privileges') || 
    f.CHECK_ID.includes('service_accounts_no_admin') || 
    f.CHECK_ID.includes('iam_editor') ||
    (f.SERVICE_NAME === 'iam' && (f.CHECK_TITLE.toLowerCase().includes('editor') || f.CHECK_TITLE.toLowerCase().includes('admin')))
  );

  if (hasPermissiveIngress && hasBroadSA) {
    sections += `
### ⚠️ TOXIC COMBINATION 1: Internet-Exposed Compute Instance with Over-Privileged Service Account (Critical Risk)

**Vulnerabilities Involved:**
1. **Permissive Network Access:** Ingress is open from the public internet (\`0.0.0.0/0\`) directly to ports 22 (SSH) or 3389 (RDP).
2. **Excessive IAM Permissions:** Compute default or attached Service Accounts are bound to administrative roles (e.g., \`Editor\` or \`Owner\`).

**Compounded Attack Vector (Kill Chain):**
1. **Initial Entry:** An attacker compromises the Compute Engine instance by brute-forcing SSH/RDP credentials or exploiting a software vulnerability.
2. **Privilege Escalation:** Once inside the VM, the attacker queries the local Metadata Server (\`http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/token\`) to retrieve the access token for the attached service account.
3. **Lateral Movement & Takeover:** Since the Service Account has project-wide \`Editor\` or administrator privileges, the attacker instantly gains the ability to provision new resources, delete data, download database backups, or hijack GKE clusters across the entire project.

**Remediation Steps:**
- **Network Hardening:** Delete the permissive firewall rule allowing \`0.0.0.0/0\`. Replace it with fine-grained rules restricted to corporate subnets or use **Identity-Aware Proxy (IAP)** to enable secure SSH/RDP without public IPs.
- **IAM Principle of Least Privilege:** Remove broad roles like \`Editor\` or \`Owner\` from Service Accounts. Instead, attach a custom Service Account with only the precise permissions required by the workload.
`;
  }

  // 2. Cloud SQL Public IP + Unenforced SSL
  const hasSqlPublicIP = Object.values(aggregatedData).some(f => 
    f.CHECK_ID.includes('public_ip') || 
    f.CHECK_TITLE.toLowerCase().includes('public ip address')
  );
  const hasSqlNoSsl = Object.values(aggregatedData).some(f => 
    f.CHECK_ID.includes('ssl_connections') || 
    f.CHECK_TITLE.toLowerCase().includes('ssl connections') || 
    f.CHECK_TITLE.toLowerCase().includes('require ssl')
  );

  if (hasSqlPublicIP && hasSqlNoSsl) {
    sections += `
### ⚠️ TOXIC COMBINATION 2: Cloud SQL Public IP Exposure with Unenforced SSL/TLS (High Risk)

**Vulnerabilities Involved:**
1. **Public IP Assigned:** The Cloud SQL instance is reachable over the public internet.
2. **SSL/TLS Unenforced:** Connections to the database do not require encryption, allowing cleartext data transmission.

**Compounded Attack Vector (Kill Chain):**
1. **Eavesdropping:** Since SSL is not enforced, an attacker positioned along the network path (e.g., via a compromised router, malicious transit ISP, or DNS spoofing) can capture database traffic in transit.
2. **Credential Sniffing:** Cleartext database credentials (passwords, usernames) are sniffed during login handshakes.
3. **Direct Database Breach:** Armed with valid credentials and a database port exposed to the public web, the attacker connects directly to the database, bypassing firewalls, to read, exfiltrate, or ransom sensitive client tables.

**Remediation Steps:**
- **Private Database Networking:** Disable the public IP of the Cloud SQL instance: \`gcloud sql instances patch [INSTANCE_NAME] --no-assign-ip\`. Connect only via **Private Service Access (PSA)** or **Private Service Connect (PSC)**.
- **Enforce TLS:** Enable SSL-only connections: \`gcloud sql instances patch [INSTANCE_NAME] --require-ssl\`.
`;
  }

  // 3. Public Storage Bucket + Lack of Log Monitoring
  const hasPublicBucket = Object.values(aggregatedData).some(f => 
    f.CHECK_ID.includes('public_access') || 
    f.CHECK_TITLE.toLowerCase().includes('public_access') ||
    f.CHECK_TITLE.toLowerCase().includes('public access')
  );
  
  if (hasPublicBucket) {
    sections += `
### ⚠️ TOXIC COMBINATION 3: Publicly Accessible Cloud Storage Bucket (High Risk)

**Vulnerabilities Involved:**
1. **Public Access Allowed:** The storage bucket is configured with \`allUsers\` or \`allAuthenticatedUsers\` permissions, allowing anonymous internet users to read or write objects.

**Compounded Attack Vector (Kill Chain):**
1. **Discovery:** Automated scanners constantly probe Google Cloud Storage buckets for public list or read permissions.
2. **Secret & Code Harvesting:** Attackers download backup ZIPs, configuration files (\`.env\`), JSON service account keys, or sensitive customer databases stored in the public bucket.
3. **Downstream Escalation:** The harvested keys or credentials are used to compromise secondary resources (e.g., third-party APIs, other databases, or GCP administrative roles).

**Remediation Steps:**
- **Enforce Public Access Prevention (PAP):** Update the bucket configuration to block public access at the project/bucket level: \`gcloud storage buckets update gs://[BUCKET_NAME] --public-access-prevention\`.
- **Remove Anonymous Bindings:** Remove \`allUsers\` and \`allAuthenticatedUsers\` from IAM policies of the bucket.
`;
  }

  if (sections === "") {
    sections = `\n*No critical multi-service toxic combinations were automatically detected during this scan. Ensure that standard network and IAM boundaries continue to be strictly maintained.*`;
  }

  return `
## ⚡ Compound Attack Paths & Toxic Combinations

Toxic combinations occur when multiple distinct security vulnerabilities, which might seem low or medium risk individually, combine to create an escalated, critical threat vector that allows an attacker to breach the project boundaries.
...
function generateMarkdownReport(aggregatedData: Record<string, AggregatedFinding>, templatePath: string): string {
  ...
  // Append Toxic Combinations
  const toxicCombinations = analyzeToxicCombinations(aggregatedData);
  finalMarkdown += toxicCombinations;
